...
import {
  ...
  FileText, ExternalLink, AlertTriangle, Wallet, MessageCircle, Car
...
async function saveField(fieldname: string) {
  if (!customerStore.customer) return
  const value = (
    fieldname === 'email_id' ? editEmail.value
    : fieldname === 'mobile_no' ? editMobile.value
    : fieldname === 'custom_whatsapp' ? editWhatsapp.value
    ...
    <div class="relative w-full max-w-md bg-white dark:bg-gray-900 shadow-2xl dark:shadow-black/50 flex flex-col overflow-hidden">
      ...
      <div v-if="customerStore.customer" class="flex-1 overflow-y-auto">
        ...
        <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800 space-y-3">
          ...
          <!-- WhatsApp -->
          <div>
            <label class="text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-1 block">{{ __('WhatsApp') }}</label>
            <div class="flex items-center gap-2">
              ...
              <input
                ...
                class="flex-1 text-sm text-gray-800 dark:text-gray-200 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400 transition-colors disabled:opacity-50 placeholder-gray-400 dark:placeholder-gray-500"
              />
              <span v-if="savingField === 'custom_whatsapp'" class="text-[10px] text-blue-500 shrink-0">{{ __('Saving...') }}</span>
              <button
        ...
        <!-- Cars -->
        <div v-if="customerStore.cars.length > 0" class="px-4 py-3 border-b border-gray-100 dark:border-gray-800">
          <div class="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">{{ __('Cars') }}</div>
          <div class="space-y-1.5">
            <button
              ...
              :class="c.name ? 'hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer' : 'cursor-default'"
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div v-if="c.notes" class="text-[10px] text-gray-400 dark:text-gray-500 mt-0.5 truncate">
                  {{ c.notes }}
                </div>
              </div>
              <ExternalLink v-if="c.name" :size="12" class="text-gray-300 dark:text-gray-600 group-hover:text-blue-400 shrink-0 mt-1" />
            </button>
          </div>
        </div>
