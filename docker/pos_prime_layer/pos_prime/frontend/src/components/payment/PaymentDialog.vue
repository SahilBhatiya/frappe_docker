...
import { Check, Banknote, CreditCard, Wallet, WalletCards, Coins, Award, Eraser, Delete, Loader2, AlertTriangle, Lock, Info, Star } from 'lucide-vue-next'
...
// ESC is handled natively by reka-ui Dialog
...
<template>
  <!-- Main Payment Dialog -->
  <Dialog :open="paymentStore.showPaymentDialog" @update:open="(val) => { if (!val) paymentStore.closePaymentDialog() }">
    <DialogContent class="sm:max-w-md max-h-[90vh] p-0 gap-0 overflow-hidden flex flex-col" :show-close-button="false">
      <div class="p-6 overflow-y-auto no-scrollbar flex-1 flex flex-col">
        <!-- Header -->
        <DialogHeader class="flex-row items-center justify-between mb-6 space-y-0">
          <div class="flex items-center gap-3">
            <div class="w-9 h-9 bg-gray-100 rounded-xl flex items-center justify-center text-gray-700">
              <WalletCards :size="17" />
            </div>
            <DialogTitle class="text-lg font-bold text-gray-900">{{ __('Payment') }}</DialogTitle>
          </div>
        </DialogHeader>
        ...
        <div v-if="applyStoreCredit" class="mb-6 px-2 space-y-2">
          ...
          <div class="flex items-center justify-between text-xs">
            <span class="text-gray-500">{{ __('Applied to this invoice') }}</span>
            <span class="font-bold text-gray-900">-{{ formatCurrency(storeCreditAppliedAmount) }}</span>
          </div>
        </div>
        ...
        <div v-if="redeemLoyalty" class="mb-6 px-2 space-y-2">
          ...
          <div v-if="loyaltyApplied" class="flex items-center justify-between text-xs">
            <span class="text-gray-500">{{ loyaltyPointsToRedeem }} {{ __('pts') }} × {{ customerStore.loyaltyData?.conversion_factor }}</span>
            <span class="font-bold text-gray-900">-{{ formatCurrency(loyaltyRedemptionAmount) }}</span>
          </div>
        </div>
  ...
  <!-- Partial Payment Confirmation (nested Dialog) -->
  <Dialog :open="showPartialConfirm" @update:open="(val) => { if (!val) showPartialConfirm = false }">
    <DialogContent class="sm:max-w-xs text-center" :show-close-button="false">
      <div class="flex flex-col items-center">
        <div class="w-12 h-12 bg-amber-50 rounded-full flex items-center justify-center mb-4">
