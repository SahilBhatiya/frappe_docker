...
<template>
	<Dialog :open="show" @update:open="(val) => { if (!val) emit('close') }">
		<!-- shadcn DialogContent handles the portal, overlay and presence gating,
		     so the dialog only mounts while `show` is true. -->
		<DialogContent
			:show-close-button="false"
			class="w-full max-w-4xl sm:max-w-4xl p-0 gap-0 max-h-[90vh] rounded-2xl ring-0 shadow-[0px_48px_100px_0px_rgba(17,12,46,0.15)] border border-zinc-200 bg-white dark:bg-gray-900 overflow-hidden flex flex-col md:flex-row"
		>
