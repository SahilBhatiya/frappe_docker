...
onMounted(async () => {
  try {
    ...
    // Auto-open shift if there's exactly one POS profile available
    if (settingsStore.posProfiles.length === 1) {
      ...
      // Attempt to auto-open with default (0) balances
      await openShift()
    } else if (settingsStore.posProfiles.length === 0) {
      error.value = 'No POS Profiles found! Server returned 0 profiles.'
    }
  } catch (err: any) {
    console.error('fetchPOSProfiles error:', err)
    error.value = `[${err.status || 'No Status'}] ${err.exc_type || 'Unknown'}: ${err.messages?.[0] || err.message || 'Failed to load POS Profiles'}`
  }
})
