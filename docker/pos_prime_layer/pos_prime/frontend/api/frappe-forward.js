// Generic reverse proxy for Frappe-owned, server-rendered paths that are NOT
// part of the SPA (e.g. /printview, /assets/frappe, /files, /private).
//
// Unlike frappe-proxy.js (which is hardcoded to /api/method/*), this forwards
// the upstream path verbatim. The path is passed via the `__fp` query param by
// the vercel.json rewrite (e.g. "/printview" -> "/api/frappe-forward?__fp=/printview"),
// and every other query param is passed through to the backend.
...
export default async function handler(req, res) {
  ...
  if (!backendUrl) {
    res.status(500).json({ error: 'FRAPPE_BACKEND_URL is not configured on Vercel' })
    return
  }
  ...
  // Upstream path comes from the rewrite; default to root so a misconfigured
  // rewrite fails loudly rather than silently proxying /api/method.
  const rawFp = req.query?.__fp
  let upstreamPath = Array.isArray(rawFp) ? rawFp[0] : rawFp || '/'
  ...
  const upstreamUrl = `${backendUrl}${upstreamPath}${query}`
