...
def get_opening_data():
    ...
    # Auto-close old open entries
    old_entries = frappe.get_all(
        "POS Opening Entry",
        filters={"status": "Open", "docstatus": 1, "period_start_date": ("<", today)},
        fields=["name"]
    )
    for entry in old_entries:
        try:
            close_shift(entry.name)
        except Exception as e:
            frappe.log_error(title="POS Prime Auto-Close", message=f"Failed to auto-close POS shift {entry.name}: {str(e)}")

    # Check for existing open entry for today
    existing = frappe.get_all(
        "POS Opening Entry",
        filters={"status": "Open", "docstatus": 1, "period_start_date": (">=", today), "user": user},
    ...
    opening_entry = existing[0] if existing else None

    # Auto-create if not exists
    if not opening_entry and profiles:
        try:
            pos_profile = profiles[0].get("name")
            company = profiles[0].get("company")
            profile_doc = frappe.get_doc("POS Profile", pos_profile)
            balance_details = [{"mode_of_payment": pm.mode_of_payment, "opening_amount": 0} for pm in profile_doc.payments]
            
            new_doc = create_opening_entry(pos_profile, company, balance_details)
            opening_entry = {
            ...
            frappe.log_error(title="POS Prime Auto-Create", message=f"Failed to auto-create POS shift: {str(e)}")
...
def check_opening_entry(user=""):
    """Check if there is an open POS Opening Entry for today."""
    validate_pos_access()
    today = frappe.utils.nowdate()
    entries = frappe.get_all(
        "POS Opening Entry",
        filters={"docstatus": 1, "status": "Open", "period_start_date": (">=", today)},
        fields=["name", "pos_profile", "company"],
        order_by="creation desc",
        limit=1
    )
