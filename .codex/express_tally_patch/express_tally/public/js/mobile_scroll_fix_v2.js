(function () {
	...
	var runtimeEventsBound = false;

	function bindRuntimeEvents() {
		if (runtimeEventsBound) return true;

		var jq = window.jQuery || window.$;
		if (!jq || !window.frappe || !frappe.router || !frappe.router.on) return false;

		jq(document).on("sidebar-expand page-change form-refresh", scheduleUnlock);
		frappe.router.on("change", scheduleUnlock);
		runtimeEventsBound = true;
		return true;
	}

	function waitForRuntimeEvents(attemptsLeft) {
		if (bindRuntimeEvents() || attemptsLeft <= 0) return;
		window.setTimeout(function () {
			waitForRuntimeEvents(attemptsLeft - 1);
		}, 200);
	}
