erpnext.PointOfSale.ItemDetails = class {
	...
	render_form(item) {
		...
		fields_to_display.forEach((fieldname, idx) => {
			...
			const field_meta =
				fieldname === "pos_total_amount"
					? this.get_total_amount_field()
					: this.item_meta.fields.find((df) => df.fieldname === fieldname);
			...
			this[`${fieldname}_control`] = frappe.ui.form.make_control({
				df: {
					...field_meta,
					onchange: function () {
						if (fieldname === "pos_total_amount") {
							if (!me.updating_total_amount_control) {
								me.update_rate_from_total_amount(this.value);
							}
						} else {
			...
			const value =
				fieldname === "pos_total_amount" ? this.get_total_amount_value(item) : item[fieldname];
	...
	get_total_amount_value(item) {
		return flt(item.amount || flt(item.qty) * flt(item.rate), 2);
	}

	update_total_amount_control(item) {
		if (!this.pos_total_amount_control) return;

		this.updating_total_amount_control = true;
		this.pos_total_amount_control.set_value(this.get_total_amount_value(item));
		this.updating_total_amount_control = false;
	}

	update_rate_from_total_amount(total_amount) {
		const qty = flt(this.qty_control && this.qty_control.get_value()) || flt(this.current_item.qty);
		if (!qty) return;

		const rate = flt(total_amount) / qty;
		return this.events.form_updated(this.current_item, "rate", rate).then(() => {
			const item_row = frappe.get_doc(this.doctype, this.name);
			const doc = this.events.get_frm().doc;
			this.rate_control && this.rate_control.set_value(item_row.rate);
			this.$item_price.html(format_currency(item_row.rate, doc.currency));
			this.render_discount_dom(item_row);
			this.update_total_amount_control(item_row);
		});
	}
	...
	bind_custom_control_change_event() {
		...
		if (this.qty_control) {
			this.qty_control.df.onchange = function () {
				if (this.value || flt(this.value) === 0) {
					me.events.form_updated(me.current_item, "qty", this.value).then(() => {
						const item_row = frappe.get_doc(me.doctype, me.name);
						me.update_total_amount_control(item_row);
					});
				}
			};
		...
		frappe.model.on(`${frm_doctype} Item`, "*", (fieldname, value, item_row) => {
			...
			if (item_row_is_being_edited && ["qty", "rate", "amount"].includes(fieldname)) {
				this.update_total_amount_control(item_row);
			}
			if (
