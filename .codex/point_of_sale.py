...
def get_conditions(search_term):
	normalized_search_term = get_normalized_pos_item_search_pattern(search_term)
	condition = "("
	condition += """item.name like {search_term}
		or item.item_name like {search_term}""".format(search_term=frappe.db.escape("%" + search_term + "%"))
	if normalized_search_term:
		condition += """
		or REGEXP_REPLACE(LOWER(item.name), '[^a-z0-9]', '') like {search_term}
		or REGEXP_REPLACE(LOWER(item.item_name), '[^a-z0-9]', '') like {search_term}""".format(
			search_term=frappe.db.escape("%" + normalized_search_term + "%")
		)
...
def get_normalized_pos_item_search_pattern(search_term):
	tokens = re.findall(r"[a-z0-9]+", (search_term or "").lower())
	return "%".join(tokens)
