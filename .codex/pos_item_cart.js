erpnext.PointOfSale.ItemCart = class {
	...
	make_cart_numpad() {
		...
		this.number_pad = new erpnext.PointOfSale.NumberPad({
			...
			css_classes: [
				...
				["", "", "", "col-span-2"],
				["", "", "", "col-span-2"],
				["col-span-5 remove-btn"],
			],
			fieldnames_map: {
				Quantity: "qty",
				Discount: "discount_percentage",
				Total: "pos_total_amount",
			},
		});
	...
	on_numpad_event($btn) {
		...
		const action_is_allowed = action_is_field_edit
			? (["rate", "pos_total_amount"].includes(current_action) && this.allow_rate_change) ||
		...
		if (action_is_field_edit) {
			if (!action_is_allowed) {
				const label =
					current_action == "rate"
						? "Rate".bold()
						: current_action == "pos_total_amount"
						? "Total".bold()
						: "Discount".bold();
	...
	toggle_numpad_field_edit(fieldname) {
		if (["qty", "discount_percentage", "rate", "pos_total_amount"].includes(fieldname)) {
